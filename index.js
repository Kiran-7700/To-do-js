...
function addItemsList() {
  ...
  flexcontainer.addEventListener("click", (e) => {
    if (e.target.classList.contains("add_todo_list_btn")) {
      ...
      document.getElementById("markdoneBtn" + markDoneValue).style.display =
        "none";
      document.getElementById("listText" + markDoneValue).style.textDecoration =
        "line-through";
...
function restoreflexcontainer() {
  ...
  try {
    document
      .getElementById("card" + cardLinkValue)
      .classList.remove("bigTodoCard");
  } catch (error) {
    console.log("");
  }
}
